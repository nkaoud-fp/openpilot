...
# Navigation test control is intentionally conservative: navd is a route-aware
# intent planner. FrogPilot only accepts left/right lane-change-like commands,
# so this layer must decide when those commands are actually useful:
#   * highway exits/forks: migrate toward the target edge early and patiently
#   * surface intersections: send one close-range directional influence command
#   * everything else: publish guidance only and let the model follow curvature
NAVIGATION_TEST_DEBUG_LOG_VERSION = 3
...
NAVIGATION_TEST_SURFACE_TURN_SECONDS = 5.0
NAVIGATION_TEST_SURFACE_TURN_DISTANCE_MIN = 25.0
NAVIGATION_TEST_SURFACE_TURN_DISTANCE_MAX = 90.0
NAVIGATION_TEST_SURFACE_TURN_LATE_LOCKOUT_DISTANCE = 8.0
NAVIGATION_TEST_SURFACE_TURN_MAX_SPEED = 18.0
NAVIGATION_TEST_SURFACE_TURN_MIN_ROUTE_CONFIDENCE = 0.45
NAVIGATION_TEST_SAME_DIRECTION_REPEAT_DISTANCE = 110.0
...
class RouteEngine:
  ...
  def update_navigation_test_command(self, action, direction="none", distance=0.0, eta_seconds=0.0, display_direction=None, error="", strategy_phase="none", strategy_constraint="none", target_lane_zone="none", lane_belief="unknown", maneuver_class="unknown", maneuver_group="none", road_context="unknown", route_confidence=0.0, command_block_reason="none", urgency=0.0, command_mode="none", turn_influence_active=False):
    ...
    command = json.dumps({
      ...
      "turnInfluenceActive": bool(turn_influence_active),
      ...
      "routeGeneration": self.navigation_test_route_generation,
      "recomputeReason": self.navigation_test_recompute_reason,
    })
  ...
  def navigation_test_maneuver_class(self, instruction):
    ...
    if "roundabout" in text or "rotary" in text:
      return "roundabout"
    if "ramp" in maneuver_type or "exit" in primary_text or "take exit" in text or "off ramp" in text:
      return "highway_exit"
  ...
  def navigation_test_maneuver_group(self, maneuver_class):
    if maneuver_class in ("highway_exit", "highway_fork"):
      return "lane_positioning"
    if maneuver_class in ("normal_turn",):
      return "surface_turn"
    if maneuver_class in ("uturn", "roundabout", "arrive"):
  ...
  def navigation_test_surface_turn_distance(self):
    v_ego = self.sm['carState'].vEgo
    return min(
      NAVIGATION_TEST_SURFACE_TURN_DISTANCE_MAX,
      max(NAVIGATION_TEST_SURFACE_TURN_DISTANCE_MIN, v_ego * NAVIGATION_TEST_SURFACE_TURN_SECONDS),
    )

  def navigation_test_surface_turn_influence_candidate(self, instruction, maneuver_class, road_context):
    if instruction is None:
      return False, "noInstruction"
    if maneuver_class != "normal_turn":
      return False, "notNormalTurn"
    if road_context != "surface":
      return False, "notSurfaceRoad"
    if self.sm['carState'].vEgo > NAVIGATION_TEST_SURFACE_TURN_MAX_SPEED:
      return False, "surfaceTurnTooFast"

    modifier = instruction.get("maneuverModifier", "").lower().replace(" ", "_")
    # Mapbox often uses slight_left/slight_right for road bends and soft forks.
    # Treat those as guidance unless they were already classified as an exit/fork.
    if modifier in ("slight_left", "slight_right"):
      return False, "slightTurnGuidanceOnly"
  ...
  def navigation_test_recent_same_direction_command(self, direction, distance_to_maneuver_along_geometry):
    if direction != self.navigation_test_last_lane_change_command_direction:
      return False
    if self.navigation_test_last_lane_change_command_at <= 0.0:
      return False
    if self.navigation_test_last_lane_change_command_distance <= 0.0:
      return False

    traveled_since_command = self.navigation_test_last_lane_change_command_distance - distance_to_maneuver_along_geometry
    return 0.0 <= traveled_since_command < NAVIGATION_TEST_SAME_DIRECTION_REPEAT_DISTANCE

  def navigation_test_guidance_result(self, action, direction, display_direction, strategy_phase, strategy_threshold, strategy_constraint, target_lane_zone, lane_belief, left_available, right_available, maneuver_class, maneuver_group, road_context, command_block_reason, urgency, command_mode="guidanceOnly", turn_influence_active=False):
    return (
      action,
      direction,
      display_direction,
  ...
  def navigation_test_strategy(self, instruction, geometry, distance_to_maneuver_along_geometry, command_distance, next_maneuver_direction="none", next_maneuver_distance_after_current=None, route_confidence=1.0):
    direction = self.navigation_test_maneuver_direction(instruction)
    display_direction = self.navigation_test_maneuver_display_direction(instruction)
    maneuver_class = self.navigation_test_maneuver_class(instruction)
    maneuver_group = self.navigation_test_maneuver_group(maneuver_class)
    ...
    def result(action, strategy_phase, strategy_threshold=0.0, block_reason="none", urgency=0.0, command_mode="none", turn_influence_active=False):
      return self.navigation_test_guidance_result(
        action,
        direction,
        display_direction,
        strategy_phase,
    ...
    if post_exit_decision is not None:
      action, recenter_direction, recenter_display_direction, strategy_phase, strategy_threshold, strategy_constraint, target_lane_zone, command_block_reason, urgency = post_exit_decision
      return self.navigation_test_guidance_result(
        action,
        recenter_direction,
        recenter_display_direction,
        strategy_phase,
        strategy_threshold,
        strategy_constraint,
        target_lane_zone,
    ...
    if direction == "none" and display_direction != "uturn":
      self.reset_navigation_test_exit_migration()
      return result("none", "none", command_mode="none")

    # Surface turns get a single close-range command. This is not lane planning;
    # it is a directional influence near an actual junction where FrogPilot can
    # use the turn signal / lane-change-like request to bias the path.
    if maneuver_group == "surface_turn":
      self.reset_navigation_test_exit_migration()
      turn_window = self.navigation_test_surface_turn_distance()
      urgency = max(0.0, min(1.0, 1.0 - distance_to_maneuver_along_geometry / max(turn_window, 1.0)))

      if distance_to_maneuver_along_geometry > turn_window:
        return result("upcoming", "surfaceTurnUpcoming", turn_window, urgency=urgency, command_mode="turnInfluence")

      candidate, block_reason = self.navigation_test_surface_turn_influence_candidate(instruction, maneuver_class, road_context)
      if not candidate:
        return result("guidanceOnly", "surfaceTurnGuidanceOnly", turn_window, block_reason, urgency, "guidanceOnly")

      if distance_to_maneuver_along_geometry <= NAVIGATION_TEST_SURFACE_TURN_LATE_LOCKOUT_DISTANCE:
        return result("guidanceOnly", "surfaceTurnTooLate", NAVIGATION_TEST_SURFACE_TURN_LATE_LOCKOUT_DISTANCE, "lateTurnInfluenceLockout", 1.0, "guidanceOnly")

      if route_confidence < NAVIGATION_TEST_SURFACE_TURN_MIN_ROUTE_CONFIDENCE:
        return result("guidanceOnly", "surfaceTurnLowConfidence", turn_window, "lowRouteConfidence", urgency, "guidanceOnly")

      if self.navigation_test_recent_same_direction_command(direction, distance_to_maneuver_along_geometry):
        return result("upcoming", "sameDirectionRepeatHold", turn_window, "sameDirectionRepeatDistance", urgency, "turnInfluence")

      allowed, block_reason = self.navigation_test_lane_command_allowed(direction)
      if not allowed:
        return result("upcoming", "turnCommandCooldown", turn_window, block_reason, urgency, "turnInfluence")

      self.record_navigation_test_lane_command(direction, distance_to_maneuver_along_geometry)
      return result("laneChange", "surfaceTurnInfluence", turn_window, "none", max(urgency, 0.7), "turnInfluence", True)

    # U-turns, roundabouts, arrival, continue, and unknown maneuvers should not
    # be converted into FrogPilot lane-change commands.
    if not self.navigation_test_is_lane_positioning_maneuver(maneuver_class):
      ...
      if distance_to_maneuver_along_geometry <= command_distance:
        urgency = min(1.0, command_distance / max(distance_to_maneuver_along_geometry, 1.0))
        return result("guidanceOnly", "guidanceOnly", command_distance, urgency=urgency, command_mode="guidanceOnly")
      return result("upcoming", "upcoming", command_distance, urgency=0.0, command_mode="guidanceOnly")
    ...
    if distance_to_maneuver_along_geometry > active_prep_distance:
      self.reset_navigation_test_exit_migration()
      return result("upcoming", "lanePositioningUpcoming", active_prep_distance, urgency=urgency, command_mode="lanePositioning")
    ...
    if conflict_soon and distance_to_maneuver_along_geometry > standard_exit_prep_distance:
      ...
      return self.navigation_test_guidance_result(
        "upcoming", direction, display_direction, "consecutiveConflictHold", active_prep_distance, strategy_constraint,
        target_lane_zone, lane_belief, left_available, right_available, maneuver_class, maneuver_group, road_context,
        "conflictingNextManeuver", urgency, "lanePositioning", False,
      )

    if distance_to_maneuver_along_geometry <= late_lockout_distance:
      return result("guidanceOnly", "maneuverLockout", late_lockout_distance, "lateLaneChangeLockout", 1.0, "guidanceOnly")

    if self.navigation_test_target_edge_reached(target_lane_zone, lane_belief):
      return result("upcoming", "targetEdgeHold", active_prep_distance, "targetEdgeReached", urgency, "lanePositioning")

    if not self.navigation_test_direction_available(direction, left_available, right_available):
      return result("upcoming", "adjacentLaneHold", active_prep_distance, "adjacentLaneUnavailable", urgency, "lanePositioning")

    if self.navigation_test_recent_same_direction_command(direction, distance_to_maneuver_along_geometry):
      return result("upcoming", "sameDirectionRepeatHold", active_prep_distance, "sameDirectionRepeatDistance", urgency, "lanePositioning")
    ...
    if not allowed:
      ...
      if conflict_soon:
        strategy_constraint = "conflictingNextManeuver"
      return self.navigation_test_guidance_result(
        "upcoming", direction, display_direction, strategy_phase, active_prep_distance, strategy_constraint,
        target_lane_zone, lane_belief, left_available, right_available, maneuver_class, maneuver_group, road_context,
        block_reason, urgency, "lanePositioning", False,
      )

    strategy_phase = "exitMigration" if distance_to_maneuver_along_geometry <= standard_exit_prep_distance else "highwayExitMigration"
    if conflict_soon:
      strategy_constraint = "conflictingNextManeuver"

    self.record_navigation_test_lane_command(direction, distance_to_maneuver_along_geometry)
    return self.navigation_test_guidance_result(
      "laneChange", direction, display_direction, strategy_phase, active_prep_distance, strategy_constraint,
      target_lane_zone, lane_belief, left_available, right_available, maneuver_class, maneuver_group, road_context,
      command_block_reason, urgency, "lanePositioning", False,
    )
  ...
  def log_navigation_test_debug(self, instruction, geometry, distance_to_maneuver_along_geometry, command_distance, action, direction, cross_track_error=None, strategy_phase="none", strategy_threshold=0.0, strategy_constraint="none", next_maneuver_direction="none", next_maneuver_distance_after_current=None, maneuver_class="unknown", maneuver_group="none", road_context="unknown", target_lane_zone="none", lane_belief="unknown", lane_left_available=None, lane_right_available=None, route_confident=False, route_confidence=0.0, route_bearing_error=None, command_block_reason="none", urgency=0.0, command_mode="none", turn_influence_active=False):
    if not self.params.get_bool("NavigationTestControl"):
      return
    ...
    current_step_error = self.path_minimum_distance(geometry) if geometry is not None else None
    ...
    row = {
      ...
      "turn_influence_active": turn_influence_active,
  ...
  def recompute_route(self):
    ...
    if new_destination != self.nav_destination:
      ...
      self.navigation_test_recompute_reason = "destinationChanged"
  ...
  def send_instruction(self):
    ...
    navigation_test_maneuver_group = self.navigation_test_maneuver_group(navigation_test_maneuver_class)
    navigation_test_road_context = self.navigation_test_road_context(navigation_test_maneuver_class)
    navigation_test_command_mode = "none"
    navigation_test_turn_influence_active = False
    ...
    if self.params.get_bool("NavigationTestControl"):
      ...
      if not navigation_test_route_confident:
        ...
        (
          ...
          navigation_test_maneuver_group,
          navigation_test_road_context,
          navigation_test_command_mode,
          navigation_test_turn_influence_active,
    ...
    if self.params.get_bool("NavigationTestControl"):
      self.update_navigation_test_command(
        ...
        maneuver_group=navigation_test_maneuver_group,
        ...
        command_mode=navigation_test_command_mode,
        turn_influence_active=navigation_test_turn_influence_active,
      )
  ...
  def should_recompute(self):
    ...
    if self.step_idx is None or self.route is None:
      self.navigation_test_recompute_reason = "noActiveRoute"
      return True
