...
class RouteEngine:
  ...
  def update_navigation_test_command(self, action, direction="none", distance=0.0, display_direction=None, maneuver_class="unknown", urgency=0.0, error="none"):
    command = json.dumps({
      "action": action,                  # 'none', 'upcoming', 'laneChange', 'prepareTurn'
      "direction": direction,            # 'left', 'right', 'none'
      "displayDirection": display_direction or direction,
      "distance": max(distance, 0.0),
      "maneuverClass": maneuver_class,   # 'highway_exit', 'normal_turn', etc.
  ...
  def navigation_test_strategy(self, instruction, distance_to_maneuver_along_geometry, road_context):
    direction = self.navigation_test_maneuver_direction(instruction)
    display_direction = self.navigation_test_maneuver_display_direction(instruction)
    maneuver_class = self.navigation_test_maneuver_class(instruction)
    v_ego = max(self.sm['carState'].vEgo, 1.0) # Prevent division by zero
    
    # Base constraints
    if direction == "none" and display_direction != "uturn":
      return "none", direction, display_direction, maneuver_class, 0.0

    # Dynamic Time-to-Maneuver (TTM)
    time_to_maneuver = distance_to_maneuver_along_geometry / v_ego

    # Configuration Thresholds (adjust these tuning values as needed)
    PREP_TIME_HIGHWAY = 45.0  # Seconds to start lane changing before an exit
    PREP_TIME_SURFACE = 20.0  # Seconds to start lane changing before a surface turn
    TURN_BIAS_TIME = 6.0      # Seconds before intersection to apply steering bias/braking

    active_prep_time = PREP_TIME_HIGHWAY if road_context == "highway" else PREP_TIME_SURFACE

    # 1. State: Upcoming (Too far to act)
    if time_to_maneuver > active_prep_time:
      return "upcoming", direction, display_direction, maneuver_class, 0.0

    # Calculate urgency scaling from 0.0 (just entered prep zone) to 1.0 (at the turn)
    urgency = min(1.0, (active_prep_time - time_to_maneuver) / active_prep_time)

    # 2. State: Highway Exits and Forks (Pure Lane Positioning)
    if self.navigation_test_is_lane_positioning_maneuver(maneuver_class):
      return "laneChange", direction, display_direction, maneuver_class, urgency

    # 3. State: Surface Street Turns and Intersections
    if time_to_maneuver <= TURN_BIAS_TIME:
      # Tell FrogPilot we are inside the junction zone
      return "prepareTurn", direction, display_direction, maneuver_class, 1.0
    else:
      # Tell FrogPilot we need to get into the correct lane for the upcoming turn
      return "laneChange", direction, display_direction, maneuver_class, urgency
