...
NAVIGATION_TEST_DEBUG_LOG_INTERVAL = 0.53b
...
NAVIGATION_TEST_DEBUG_LOG_VERSION = 4
...
class RouteEngine:
  ...
  def turn_slowdown_speed(self, maneuver_class, distance_to_maneuver):
    ...
    # Let's assume a comfortable deceleration of 1.5 m/s^2
    comfortable_decel = 1.5 
    v_ego = max(self.sm['carState'].vEgo, 0.0)
    
    # Calculate exactly how much distance we need to brake from current speed to min speed
    # d = (v_i^2 - v_f^2) / (2 * a)
    required_braking_distance = (v_ego**2 - TURN_SLOWDOWN_MIN_SPEED_MS**2) / (2 * comfortable_decel)
    required_braking_distance = max(required_braking_distance, 50.0) # Always give at least 50m
    
    if distance_to_maneuver is None or distance_to_maneuver > required_braking_distance:
        return 0.0
  ...
  def navigation_test_lane_availability(self):
    ...
    try:
        ...
        # 1. Check outer lane line probabilities (positive indicators of adjacent lanes)
        ...
        if len(lane_line_probs) >= 4:
            ...
            # If the model is confident an outer line exists, a lane likely exists
        ...
        # 2. Check Physical Road Edges (negative indicators / hard boundaries)
        # roadEdges[0] = Left Edge | roadEdges[1] = Right Edge
        # y values are lateral distance: Positive is Left, Negative is Right.
        road_edges = list(model.roadEdges)
        if len(road_edges) >= 2:
            # We look at the closest predicted point (index 0 of the y-array)
            left_edge_y = road_edges[0].y[0] if len(road_edges[0].y) > 0 else None
            right_edge_y = road_edges[1].y[0] if len(road_edges[1].y) > 0 else None
            
            # Standard lane width is ~3.5 meters. 
            # If the edge is closer than 3.5m, an adjacent lane physically cannot exist.
            if left_edge_y is not None and left_edge_y < 3.5:
                left_available = False  # Overrides line probabilities; you are at the physical edge
                
            if right_edge_y is not None and right_edge_y > -3.5:
                right_available = False # Overrides line probabilities; you are at the physical edge
