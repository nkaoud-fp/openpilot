...
NAVIGATION_TEST_SHARED_DESTINATION_DATABASE_URL = os.environ.get("NAVIGATION_TEST_SHARED_DESTINATION_DATABASE_URL", NAVIGATION_TEST_SHARED_DESTINATION_DATABASE_URL_strn3 + NAVIGATION_TEST_SHARED_DESTINATION_DATABASE_URL_strn1 + NAVIGATION_TEST_SHARED_DESTINATION_DATABASE_URL_strn4 + NAVIGATION_TEST_SHARED_DESTINATION_DATABASE_URL_strn2 )
...
NAVIGATION_TEST_EXIT_PREP_MAX_LANE_CHANGES = 5 # 4
...
NAVIGATION_TEST_HIGHWAY_EXIT_PREP_DISTANCE_MIN = 1500 # Has toggle
NAVIGATION_TEST_HIGHWAY_EXIT_PREP_DISTANCE_MAX = 5000 # Has toggle
...
NAVIGATION_TEST_TURN_COMMAND_DISTANCE_MIN = 35.0 # Has toggle
NAVIGATION_TEST_TURN_COMMAND_SECONDS = 8.0 # Has toggle
NAVIGATION_TEST_SURFACE_TURN_PREP_SECONDS = 18.0
NAVIGATION_TEST_SURFACE_TURN_PREP_DISTANCE_MIN = 120.0 # Has toggle
NAVIGATION_TEST_SURFACE_TURN_PREP_DISTANCE_MAX = 650.0 # Has toggle
NAVIGATION_TEST_LATE_LANE_CHANGE_LOCKOUT_SECONDS = 3.0
NAVIGATION_TEST_LATE_LANE_CHANGE_LOCKOUT_DISTANCE_MIN = 35.0 # Has toggle
NAVIGATION_TEST_LANE_SAMPLE_SECONDS = 0.5
NAVIGATION_TEST_LANE_SAMPLE_RETENTION_SECONDS = 5 # 2.0
NAVIGATION_TEST_DEBUG_LOG_DIR = "/data/media/0/navigation_test_logs"
NAVIGATION_TEST_DEBUG_LOG_INTERVAL = 0.5
TURN_SLOWDOWN_MIN_SPEED_MS = 25.0 / 3.6   # Has toggle
...
class RouteEngine:
  ...
  def navigation_test_has_intermediate_exit_ahead(self, direction, distance_to_maneuver_along_geometry):
    if self.step_idx is None or direction not in ("left", "right"):
      return False
    if self.step_idx >= len(self.navigation_test_step_intermediate_exits):
      return False
    for exit_info in self.navigation_test_step_intermediate_exits[self.step_idx]:
  ...
  def navigation_test_strategy(self, instruction, geometry, distance_to_maneuver_along_geometry, command_distance, next_maneuver_direction="none", next_maneuver_distance_after_current=None):
    ...
    has_intermediate = self.navigation_test_has_intermediate_exit_ahead(direction, distance_to_maneuver_along_geometry)
    if has_intermediate:
      lanes_to_edge = self.navigation_test_lanes_to_target_edge(direction)
      if lanes_to_edge is not None and lanes_to_edge <= 1:
        return "upcoming", direction, display_direction, "targetEdgeHold", active_prep_distance, "intermediateExitEdgeHold"
  ...
  def _check_and_apply_route_thread(self):
    ...
    if len(r.get('routes', [])):
      ...
      for step_idx, step in enumerate(self.route):
        ...
        step_geom = self.route_geometry[step_idx] if step_idx < len(self.route_geometry) else []
        cumulative_dists = [0.0]
        for gi in range(1, len(step_geom)):
          cumulative_dists.append(cumulative_dists[-1] + step_geom[gi - 1].distance_to(step_geom[gi]))
        step_total_dist = cumulative_dists[-1] if cumulative_dists else 0.0

        for idx, intersection in enumerate(intersections):
          if idx == len(intersections) - 1:
            break
    ...
    if self.params.get_bool("NavigationTestControl"):
      ...
      if cross_track_error is not None and cross_track_error > NAVIGATION_TEST_MAX_COMMAND_CROSS_TRACK_ERROR:
        ...
        if not current_actionable:
          ...
          if self.navigation_test_has_intermediate_exit_ahead(navigation_test_direction, distance_to_maneuver_along_geometry):
