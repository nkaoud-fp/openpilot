...
##
## EITHER run with no arguments (defaults to toyota_nodsu_pt_generated):
##   python3 /data/dbc_discovery.py
##
## OR specify a different source DBC:
##   python3 /data/dbc_discovery.py another_source_file
#
#  This script discovers unknown CAN messages and intelligently creates or updates
#  a skeleton DBC file for them.
#
...
def main():
    parser = argparse.ArgumentParser(
        description="Discover and log unknown CAN messages to a sorted DBC file.",
        formatter_class=argparse.RawTextHelpFormatter
    )
    # --- MODIFIED LINE ---
    # Made the source_dbc argument optional with a default value
    parser.add_argument("source_dbc", nargs='?', default="toyota_nodsu_pt_generated", 
                        help="Name of the source DBC file (default: toyota_nodsu_pt_generated)")
    parser.add_argument("-o", "--output", default="/data/new_toyota_messages.dbc", 
                        help="Path to the output DBC file (default: /data/new_toyota_messages.dbc)")
    args = parser.parse_args()

    source_dbc_name = args.source_dbc.replace('.dbc', '')
    opendbc_dir = os.path.dirname(opendbc.__file__)
    source_dbc_path = os.path.join(opendbc_dir, f"{source_dbc_name}.dbc")
    
    print(f"Loading source DBC: {source_dbc_path}")
    source_ids = get_message_ids_from_file(source_dbc_path)
    if not source_ids:
        print(f"Error: Source DBC '{source_dbc_name}' not found or is empty.")
        return

    print(f"Target DBC file: {args.output}")
    existing_generated_ids = get_message_ids_from_file(args.output)
    ...
    try:
        ...
        update_dbc_file(newly_discovered_msgs, args.output)
        print("----------------------------------")
