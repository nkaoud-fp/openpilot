...
def monitor_adaptive(dbc_name, messages_to_monitor, signals_map, log_file_handle, delay):
    """
    Monitors the live CAN bus, identifying and suppressing signals that change too frequently.
    """
    can_parser = CANParser(dbc_name, messages_to_monitor, 0)
    can_sock = messaging.sub_sock('can', timeout=100)
    last_values = {}
    
    # --- State for new suppression logic ---
    change_timestamps = defaultdict(list)  # Tracks recent change times for each signal
    chatty_signals = set()
    DETECTION_WINDOW = 4.0  # seconds
    REPETITION_THRESHOLD = 4  # Number of changes to trigger suppression

    print(f"\nMonitoring in Adaptive mode.")
    print(f"Signals changing {REPETITION_THRESHOLD} or more times within {DETECTION_WINDOW}s will be permanently suppressed.")
    print(f"Using delay: {delay}s. Press Ctrl+C to exit.")
    ...
    while True:
        ...
        if changes:
            ...
            for signal_key, values in changes.items():
                # 1. Ignore signals already flagged as chatty
                if signal_key in chatty_signals:
                    continue

                # 2. Record this change and check if it meets the suppression threshold
                timestamps = change_timestamps[signal_key]
                
                # Add current change time
                timestamps.append(current_time)
                
                # Prune timestamps older than the detection window
                detection_start_time = current_time - DETECTION_WINDOW
                timestamps = [t for t in timestamps if t >= detection_start_time]
                change_timestamps[signal_key] = timestamps
                
                # Check if the signal is now chatty
                if len(timestamps) >= REPETITION_THRESHOLD:
                    chatty_signals.add(signal_key)
                    del change_timestamps[signal_key]  # Clean up
                    
                    suppress_msg_term = (f"[{time.strftime('%H:%M:%S')}] {bcolors.WARNING}Suppressing noisy signal:"
                                         f" {bcolors.OKCYAN}{signal_key}{bcolors.ENDC}")
                    suppress_msg_log = f"[{time.strftime('%H:%M:%S')}] Suppressing noisy signal: {signal_key}"
                    
                    print(suppress_msg_term)
                    log_file_handle.write(suppress_msg_log + "\n")
                    continue

                # 3. If not suppressed, this is a normal change. Log it.
                changes_to_log[signal_key] = values

            if changes_to_log:
                format_and_log_changes(changes_to_log, log_file_handle)

            # Always update last_values with all changes to keep state correct
            for signal_key, (last_val, new_val) in changes.items():
                last_values[signal_key] = new_val
...
def get_user_settings():
    ...
    while mode not in ['1', '2', '3', '4']:
        mode = input("\nSelect mode: (1) Continuous Log (2) Interactive Action (3) Guided Discovery (4) Adaptive Log? [1/2/3/4]: ")
    ...
    delay = 1.2
    if mode in ['1', '4']: # Added adaptive mode to delay prompt
        try:
...
def main():
    ...
    try:
        with open(LOG_FILENAME, 'a', encoding='utf-8') as log_file_handle:
            ...
            if mode == '4':
                monitor_adaptive(dbc_name, messages_to_monitor, signals_map, log_file_handle, settings["delay"])
            elif mode == '3':
