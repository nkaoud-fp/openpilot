...
class Parser:
  ...
  def _parse_plan_like(self, name, outs):
    if self.check_missing(outs, name):
      return

    raw = outs[name]
    is_single_hypothesis = raw.shape[1] == 2 * ModelConstants.IDX_N * ModelConstants.PLAN_WIDTH
    in_N, out_N = (0, 0) if is_single_hypothesis else (ModelConstants.PLAN_MHP_N, ModelConstants.PLAN_MHP_SELECTION)
    self.parse_mdn(name, outs, in_N=in_N, out_N=out_N,
  ...
  def split_outputs(self, outs: dict[str, np.ndarray]) -> None:
    ...
    self._parse_plan_like('plan', outs)
    self._parse_plan_like('planplus', outs)
  ...
  def parse_off_policy_outputs(self, outs: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
    # Compatibility-first path for split models: parse only the common planning outputs
    # that may be emitted by the off-policy head, without requiring policy-only tensors.
    self.split_outputs(outs)
    if 'lat_planner_solution' in outs:
      self.parse_mdn('lat_planner_solution', outs, in_N=0, out_N=0,
                     out_shape=(ModelConstants.IDX_N, ModelConstants.LAT_PLANNER_SOLUTION_WIDTH))
