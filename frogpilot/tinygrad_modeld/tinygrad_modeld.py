...
def _artifact_layout(vision_meta: Path, vision_pkl: Path, policy_meta: Path, policy_pkl: Path,
                     off_policy_meta: Path, off_policy_pkl: Path) -> str | None:
  legacy_ready = all(path.is_file() for path in (vision_meta, vision_pkl, policy_meta, policy_pkl))
  split_ready = legacy_ready and all(path.is_file() for path in (off_policy_meta, off_policy_pkl))
  ...
  def __init__(self, context: CLContext):
    ...
    for candidate in selected_candidates:
      ...
      layout = _artifact_layout(candidate_paths[5], candidate_paths[2], candidate_paths[6], candidate_paths[3],
                                candidate_paths[7], candidate_paths[4])
    ...
    def ensure_artifact(path: Path, optional: bool = False) -> Path | None:
      if path.is_file():
        return path
    ...
    POLICY_METADATA_PATH = ensure_artifact(POLICY_METADATA_PATH)
    ...
    POLICY_PKL_PATH = ensure_artifact(POLICY_PKL_PATH)
