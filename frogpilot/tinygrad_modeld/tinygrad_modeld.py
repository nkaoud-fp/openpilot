...
from openpilot.frogpilot.common.frogpilot_variables import MODELS_PATH, get_frogpilot_toggles, params_memory
from openpilot.frogpilot.tinygrad_modeld.constants import ModelConstants, Plan
from openpilot.frogpilot.tinygrad_modeld.fill_model_msg import PublishState, fill_model_msg, fill_pose_msg, get_curvature_from_output
...
try:
  ...
  from openpilot.frogpilot.tinygrad_modeld.models.commonmodel_pyx import CLContext, DrivingModelFrame
...
class FrameMeta:
  ...
  def __init__(self, vipc=None):
    if vipc is not None:
      self.frame_id = vipc.frame_id
      self.timestamp_sof = vipc.timestamp_sof
      self.timestamp_eof = vipc.timestamp_eof
...
class ModelState:
  ...
  def __init__(self, context: CLContext):
    ...
    if current_model_param != model_id:
    ...
    def ensure_artifact(path: Path, optional: bool = False, alternates: list[Path] | None = None) -> Path | None:
      ...
      if use_builtin_model:
        raise FileNotFoundError(
          ...
          "Rebuild model artifacts locally and deploy them."
        )
    ...
    if not model_version:
      ...
      try:
        ...
        if not model_version:
          if use_builtin_model:
            builtin_layout = _artifact_layout(vision_metadata_path, vision_pkl_path,
                                              policy_metadata_path, policy_pkl_path,
  ...
  def run(self, bufs: dict[str, VisionBuf], transforms: dict[str, np.ndarray],
    ...
    self.numpy_inputs[self.desire_key][:] = self.full_desire.reshape(
    ...
    if TICI:
      for key in imgs_cl:
        if key not in self.vision_inputs:
          self.vision_inputs[key] = qcom_tensor_from_opencl_address(imgs_cl[key].mem_address,
...
def main(demo=False):
  ...
  sm = SubMaster(["deviceState", "carState", "roadCameraState", "liveCalibration", "driverMonitoringState", "carControl", "liveDelay", "frogpilotPlan", "frogpilotRadarState"])
  ...
  car_name = getattr(CP, "brand", None) or getattr(CP, "carName", "unknown")
  cloudlog.info("tinygrad_modeld got CarParams: %s", car_name)
  ...
  while True:
    ...
    if use_extra_client:
      ...
      if abs(meta_main.timestamp_sof - meta_extra.timestamp_sof) > 10000000:
        cloudlog.error(f"frames out of sync! main: {meta_main.frame_id} ({meta_main.timestamp_sof / 1e9:.5f}), extra: {meta_extra.frame_id} ({meta_extra.timestamp_sof / 1e9:.5f})")
    else:
      buf_extra = buf_main
    ...
    if model_output is not None:
      ...
      action = get_action_from_model(model_output, prev_action, lat_delay + DT_MDL, long_delay + DT_MDL,
                                     v_ego, model.mlsim, model.is_v9, frogpilot_toggles)
      prev_action = action
      ...
      DH.update(sm['carState'], sm['carControl'].latActive, lane_change_prob, sm['frogpilotPlan'], sm['frogpilotRadarState'], frogpilot_toggles)
