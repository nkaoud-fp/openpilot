...
class ModelManager:
  ...
  def _all_tinygrad_file_entries(self):
    seen = set()
    combined = []
    for filename, description in LEGACY_TINYGRAD_FILES + SPLIT_TINYGRAD_FILES:
...
################# added to compile onnx ###############
  ...
  def download_and_compile_onnx(self, model_to_download):
    ...
    expected_entries = SPLIT_TINYGRAD_FILES if onnx_layout == SPLIT_ONNX_FILES else LEGACY_TINYGRAD_FILES
