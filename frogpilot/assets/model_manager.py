...
from openpilot.frogpilot.common.frogpilot_utilities import delete_file, extract_tar, load_json_file, update_json_file, run_cmd
...
SPLIT_TINYGRAD_FILES = [
  ("driving_vision_metadata.pkl", "vision metadata"),
  ("driving_vision_tinygrad.pkl", "vision model"),
  ("driving_policy_metadata.pkl", "on-policy metadata"),
  ("driving_policy_tinygrad.pkl", "on-policy model"),
  ("driving_off_policy_metadata.pkl", "off-policy metadata"),
  ("driving_off_policy_tinygrad.pkl", "off-policy model"),
]

SPLIT_TINYGRAD_FILES_ON_POLICY = [
  ("driving_vision_metadata.pkl", "vision metadata"),
  ("driving_vision_tinygrad.pkl", "vision model"),
  ("driving_on_policy_metadata.pkl", "on-policy metadata"),
  ("driving_on_policy_tinygrad.pkl", "on-policy model"),
  ("driving_off_policy_metadata.pkl", "off-policy metadata"),
  ("driving_off_policy_tinygrad.pkl", "off-policy model"),
]
...
class ModelManager:
  ...
  def _all_tinygrad_file_entries(self):
    seen = set()
    combined = []
    for filename, description in LEGACY_TINYGRAD_FILES + SPLIT_TINYGRAD_FILES + SPLIT_TINYGRAD_FILES_ON_POLICY:
      if filename not in seen:
        combined.append((filename, description))
        seen.add(filename)
    return combined

  def _layout_name(self, file_entries):
    if file_entries == SPLIT_TINYGRAD_FILES_ON_POLICY:
      return "split_on_policy"
    if file_entries == SPLIT_TINYGRAD_FILES:
      return "split_policy"
    if file_entries == LEGACY_TINYGRAD_FILES:
      return "legacy"
    return "unknown"

  def _all_tinygrad_filenames(self):
    return [filename for filename, _ in self._all_tinygrad_file_entries()]

  def _model_has_all_files(self, model_name, file_entries):
    return all((MODELS_PATH / f"{model_name}_{filename}").is_file() for filename, _ in file_entries)

  def _get_local_layout_entries(self, model_name):
    if self._model_has_all_files(model_name, SPLIT_TINYGRAD_FILES_ON_POLICY):
      return SPLIT_TINYGRAD_FILES_ON_POLICY
    if self._model_has_all_files(model_name, SPLIT_TINYGRAD_FILES):
      return SPLIT_TINYGRAD_FILES
    if self._model_has_all_files(model_name, LEGACY_TINYGRAD_FILES):
      return LEGACY_TINYGRAD_FILES
    return None

  def _get_remote_layout_entries(self, model_name, all_model_sizes):
    split_on_policy_names = [f"{model_name}_{filename}" for filename, _ in SPLIT_TINYGRAD_FILES_ON_POLICY]
    split_names = [f"{model_name}_{filename}" for filename, _ in SPLIT_TINYGRAD_FILES]
    legacy_names = [f"{model_name}_{filename}" for filename, _ in LEGACY_TINYGRAD_FILES]

    split_on_policy_ok = all(int(all_model_sizes.get(name, 0)) > 0 for name in split_on_policy_names)
    split_ok = all(int(all_model_sizes.get(name, 0)) > 0 for name in split_names)
    legacy_ok = all(int(all_model_sizes.get(name, 0)) > 0 for name in legacy_names)
  ...
  def _get_best_layout_entries(self, model_name, all_model_sizes=None):
    local_entries = self._get_local_layout_entries(model_name)
    if local_entries is not None:
      return local_entries

    if all_model_sizes is not None:
      remote_entries = self._get_remote_layout_entries(model_name, all_model_sizes)
      if remote_entries is not None:
        return remote_entries

    return LEGACY_TINYGRAD_FILES

  def _delete_model_artifacts(self, model_name):
    delete_file(MODELS_PATH / f"{model_name}.thneed")
    for filename in self._all_tinygrad_filenames():
      delete_file(MODELS_PATH / f"{model_name}_{filename}")

  def _discover_downloaded_model_names(self):
    models = set()
    ...
    for file_path in MODELS_PATH.iterdir():
      ...
      for filename in self._all_tinygrad_filenames():
        ...
        if file_path.name.endswith(suffix):
          models.add(file_path.name[:-len(suffix)])
          break
  ...
  def _detect_remote_onnx_layout(self, model_to_download):
    split_on_policy_urls = [f"{MODELS_SOURCE_RAW}/{model_to_download}_{name}" for name in SPLIT_ONNX_FILES_ON_POLICY]
    ...
    split_ok = all(get_remote_file_size(url, self.session) > 0 for url in split_urls)
    if split_ok:
  ...
  def check_models(self, boot_run, repo_url):
    downloaded_models = self._discover_downloaded_model_names()

    for model_name in downloaded_models:
      if model_name not in set(self.available_models):
        print(f"Removing outdated model: {model_name}")
        self._delete_model_artifacts(model_name)
    ...
    for model in self.available_models:
      if self.is_tinygrad_model(model):
        ...
        expected_entries = self._get_best_layout_entries(model, model_sizes)

        model_missing = False
        model_outdated = False

        for filename, _ in expected_entries:
        ...
        if model_missing or model_outdated:
          ...
          self._delete_model_artifacts(model)
  ...
  def copy_default_model(self):
    ...
    for filename, description in self._all_tinygrad_file_entries():
  ...
  def download_all_models(self):
    ...
    for model in self.available_models:
      ...
      if self.is_tinygrad_model(model):
        ...
        already_downloaded = self._get_local_layout_entries(model) is not None
  ...
  def download_model(self, model_to_download):
    ...
    if self.is_tinygrad_model(model_to_download):
      ...
      expected_entries = self._get_remote_layout_entries(model_to_download, all_model_sizes)

      if expected_entries is None:
        print(f"Compiled files missing or incomplete on remote. Attempting ONNX fallback for {model_to_download}...")
        self.download_and_compile_onnx(model_to_download)
        return

      print(f"Using {self._layout_name(expected_entries)} layout for {model_to_download}")
      tinygrad_filenames = [f"{model_to_download}_{file_key}" for file_key, _ in expected_entries]
      ...
      for (file_key, description), part_bytes, (primary_url, fallback_url) in zip(expected_entries, file_sizes, file_sources):
...
################# added to compile onnx ###############
  ...
  def download_and_compile_onnx(self, model_to_download):
    ...
    onnx_layout = self._detect_remote_onnx_layout(model_to_download)
    if onnx_layout is None:
      handle_error(
        None,
        "Missing uncompiled model files...",
        f"Could not find a valid legacy or split ONNX layout for {model_to_download}",
    ...
    print(f"Using {'split_on_policy' if onnx_layout == SPLIT_ONNX_FILES_ON_POLICY else 'split_policy' if onnx_layout == SPLIT_ONNX_FILES else 'legacy'} ONNX layout for {model_to_download}")
    onnx_filenames = [f"{model_to_download}_{name}" for name in onnx_layout]
    ...
    if onnx_layout == SPLIT_ONNX_FILES_ON_POLICY:
      expected_entries = SPLIT_TINYGRAD_FILES_ON_POLICY
    elif onnx_layout == SPLIT_ONNX_FILES:
      expected_entries = SPLIT_TINYGRAD_FILES
    else:
      expected_entries = LEGACY_TINYGRAD_FILES
