...
# Backward-compatibility alias for legacy callers that still import TINYGRAD_FILES.
TINYGRAD_FILES = LEGACY_TINYGRAD_FILES

def all_tinygrad_file_entries():
  seen = set()
  combined = []
...
def get_tinygrad_layout(model_name):
  if all((MODELS_PATH / f"{model_name}_{filename}").is_file() for filename, _ in SPLIT_TINYGRAD_FILES_ON_POLICY):
    return "split_on_policy"
  if all((MODELS_PATH / f"{model_name}_{filename}").is_file() for filename, _ in SPLIT_TINYGRAD_FILES):
    return "split"
  if all((MODELS_PATH / f"{model_name}_{filename}").is_file() for filename, _ in LEGACY_TINYGRAD_FILES):
    return "legacy"
  return None

def model_has_complete_tinygrad_set(model_name):
  return get_tinygrad_layout(model_name) is not None

def get_downloaded_models(available_models):
  return [
    model for model in available_models
    if model and (
      (MODELS_PATH / f"{model}.thneed").is_file() or
      model_has_complete_tinygrad_set(model)
    )
  ]
...
frogpilot_default_params: list[tuple[str, str | bytes, int, str]] = [
  ...
  ("NavigationTestEmailSMTPHost", "", 3, ""),
  ("NavigationTestEmailSMTPPassword", "", 3, ""),
  ("NavigationTestEmailSMTPPort", "", 3, ""),
...
class FrogPilotVariables:
  ...
  def update(self, holiday_theme, started):
    ...
    downloaded_models = get_downloaded_models(toggle.available_models.split(","))
