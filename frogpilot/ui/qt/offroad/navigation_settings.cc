...
FrogPilotNavigationPanel::FrogPilotNavigationPanel(FrogPilotSettingsWindow *parent) : FrogPilotListWidget(parent), parent(parent) {
  ...
  QObject::connect(selectNavdButton, &ButtonControl::clicked, [this]() {
    ...
    // Disable navigation before swapping the file so the UI stops publishing
    // instructions from the old navd. NavigationTestControl is the real
    // enable flag (set by the onroad NAV button); NavDestination holds the
    // coordinates. We deliberately do not restore session state after the
    // swap — user must re-tap NAV and pick a destination again.
    // const bool savedNavEnabled = params.getBool("NavigationTestControl");
    // const std::string savedNavDestination = params.get("NavDestination");
    // const std::string savedNavDestinationWaypoints = params.get("NavDestinationWaypoints");
    // const std::string savedNavSelectedDestination = params.get("NavigationTestSelectedDestination");
    ...
    // Restore lambda removed — see comment above; we do not restore session.
    // auto restoreNavSession = [&]() {
    //   if (!savedNavSelectedDestination.empty()) {
    //     params.put("NavigationTestSelectedDestination", savedNavSelectedDestination);
    //   }
    //   if (!savedNavDestination.empty()) {
    //     params.put("NavDestination", savedNavDestination);
    //   }
    //   if (!savedNavDestinationWaypoints.empty()) {
    //     params.put("NavDestinationWaypoints", savedNavDestinationWaypoints);
    //   }
    //   if (savedNavEnabled) {
