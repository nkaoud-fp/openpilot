...
FrogPilotVisualsPanel::FrogPilotVisualsPanel(FrogPilotSettingsWindow *parent) : FrogPilotListWidget(parent), parent(parent) {
  ...
  const std::vector<std::tuple<QString, QString, QString, QString>> visualToggles {
    ...
    {"LongDebugShowConfidence", tr("Model Confidence Strip (top band)"), tr("<b>Show a colored strip across the top of the graph</b> representing the model's confidence class each frame: <b>green</b> = high confidence, <b>yellow</b> = moderate, <b>red</b> = low. Based on the same disengagement-probability score shown on the comma4 device."), ""},
    {"LongDebugShowEngProb", tr("Engaged Probability (magenta)"), tr("<b>Show the model's raw engagement probability</b> (0–1) as a continuous line. 1.0 means the model is fully confident it should be driving; drops toward 0 when it's uncertain. The confidence strip is the thresholded version of this score."), ""},
    ...
    {"LaneIndicator", tr("Lane Indicator"), tr("<b>Show which lane you're in</b> as a small pill above the navigation labels, with the model's confidence (H/M/L)."), ""},
